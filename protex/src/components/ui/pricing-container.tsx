    ...
    >
      <h1
        className="text-3xl sm:text-4xl lg:text-5xl font-black text-white 
                bg-gradient-to-r from-gray-900 to-gray-800 px-8 py-4 rounded-xl border-4 border-white/20
                shadow-[8px_8px_0px_0px_rgba(255,255,255,0.1),_15px_15px_15px_-3px_rgba(0,0,0,0.3)]
                transform transition-transform hover:translate-x-1 hover:translate-y-1 mb-3 relative
                before:absolute before:inset-0 before:bg-black/50 before:rounded-xl before:blur-sm before:-z-10"
      >
        {title}
      </h1>
      <motion.div
        className="h-2 bg-gradient-to-r from-white/20 via-gray-400/20 to-white/20 rounded-full"
        initial={{ scaleX: 0 }}
        animate={{ scaleX: 1 }}
...
const PricingToggle = ({
  ...
  <div className="flex justify-center items-center gap-4 mb-8 relative z-10">
    <span
      className={`text-gray-400 font-medium ${!isYearly ? "text-white" : ""}`}
    >
      Monthly
    </span>
    <motion.button
...
const PricingCard = ({
  ...
  return (
    <motion.div
      ...
      className={`relative w-full bg-black/40 rounded-xl p-6 border-3 border-white/20
...
export const PricingContainer = ({
  ...
  return (
    <div
      className={`min-h-screen bg-black p-4 sm:p-6 lg:p-8 relative overflow-hidden rounded-[12px] ${className}`}
    >
